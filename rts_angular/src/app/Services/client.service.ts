...
export class ClientService {
    ...
    editClient(editClient) {
        ...
        return this.http.post(ApiUrl.BaseUrl + ApiUrl.UpdateClient, editClient,
            { headers: headers })
            .map(res => {
                const responseToken = res.headers.get('refresh-token');
                localStorage.setItem('id_token', responseToken);
    ...
    getClientRequirements(userId) {
        ...
        return this.http.post(ApiUrl.BaseUrl + ApiUrl.GetClientRequirements, userId,
            { headers: headers })
            .map(res => {
                const responseToken = res.headers.get('refresh-token');
                localStorage.setItem('id_token', responseToken);
    ...
    getClientById(clientId) {
        ...
        return this.http.post(ApiUrl.BaseUrl + ApiUrl.GetClientById, clientId,
            { headers: headers })
            .map(res => {
                const responseToken = res.headers.get('refresh-token');
                localStorage.setItem('id_token', responseToken);
